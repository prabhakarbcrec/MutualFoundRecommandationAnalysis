...
public class CalCulation_service {
	...
	/**
	 * 
	 * @param borrowings   list data is coming inside from main class
	 * @param reserves     list data is coming from main class
	 * @param shareCapital list this will calculate DeRatios using all this list
	 *                     value and sending it back to main class
	 * @return
	 */
	public static float Service_processData_for_DeRatios(ArrayList<String> borrowings, ArrayList<String> reserves,
			ArrayList<String> shareCapital) {
		System.out.println("borrowings: "+borrowings);
		System.out.println("reserves: "+reserves);
		System.out.println("shareCapital: "+shareCapital);
		float DeRotio = 0;
		try {
			if (!borrowings.isEmpty() && !shareCapital.isEmpty() && !reserves.isEmpty()) {
				float borrowing = Float.valueOf(borrowings.get(borrowings.size() - 1));
				float shareCapitallocal = Float.valueOf(shareCapital.get(shareCapital.size() - 1));
				float reservelocal = Float.valueOf(reserves.get(reserves.size() - 1));
				DeRotio = (borrowing / (reservelocal + shareCapitallocal));
				System.out.println("borrowing: " + borrowing + "  shareCapitallocal: " + shareCapitallocal
						+ "  reservelocal: " + reservelocal + "  finalValueDeRatio:  " + DeRotio);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
